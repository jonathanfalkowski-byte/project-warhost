...
2. Staff the playbook's fixed action stops by dragging formations from the roster or clicking a stop to choose. Every formation exposes a neutral tactical vocabulary (`CREATES` and `USES`) without a preferred answer. Adjacent placements can reveal named condition handoffs—such as `DISPLACED → KILL ZONE`—or remain independent actions.
