...
- Player-facing formation names must identify a recognizable battlefield archetype before introducing setting flavor. Prefer names such as `GRAV-SNARE TANK`, `INCINERATOR SQUAD`, `BREACHER WALKER`, `BASTION TANK`, and `RECOVERY CARRIER` over opaque proper nouns; descriptions must state the unit's battlefield job in one plain-language sentence and remain short enough to display without truncation.
- When an optional rescue succeeds but its Recovery Carrier is later cut off, the debrief must state both events in their causal order so the successful objective and the formation's fate do not appear contradictory.
