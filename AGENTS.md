...
- Player-facing formation names must identify a recognizable battlefield archetype before introducing setting flavor. Prefer names such as `GRAV-SNARE TANK`, `INCINERATOR SQUAD`, `BREACHER WALKER`, `BASTION BATTLE TANK`, and `ARMOURED RECOVERY CARRIER` over opaque proper nouns; descriptions must state the unit's battlefield job in one plain-language sentence.
